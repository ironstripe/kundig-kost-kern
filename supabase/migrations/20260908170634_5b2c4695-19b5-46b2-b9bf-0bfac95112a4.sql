ALTER TABLE public.import_jobs ALTER COLUMN menu_card_id DROP NOT NULL;
  ...
  FOR r IN SELECT * FROM jsonb_array_elements(COALESCE(_payload->'rows','[]'::jsonb)) LOOP
    ...
    ELSIF v_action = 'update' THEN
      ...
      UPDATE public.ingredients SET
          name = btrim(r->>'name'), category = btrim(r->>'category'), supplier = NULLIF(btrim(COALESCE(r->>'supplier','')),''),
