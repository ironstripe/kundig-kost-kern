  ...
  return (
    <TooltipProvider delayDuration={200}>
      <div className="flex min-h-screen overflow-x-hidden bg-background">
        {/* Desktop sidebar – slides out to the left when collapsed */}
        <aside
          id={DESKTOP_NAV_ID}
          inert={!expanded || undefined}
          aria-hidden={!expanded || undefined}
          className={cn(
            "sticky top-0 hidden h-screen shrink-0 overflow-hidden border-sidebar-border lg:block",
            hydrated && "transition-[width] duration-200 ease-out motion-reduce:transition-none",
            expanded ? "w-60 border-r" : "w-0 border-r-0",
          )}
        >
          <div
            className={cn(
              "h-full w-60",
              hydrated && "transition-transform duration-200 ease-out motion-reduce:transition-none",
          ...
          <header className="sticky top-0 z-20 flex h-16 items-center justify-between border-b border-border bg-background/95 px-4 backdrop-blur sm:px-6 lg:px-8">
            <div className="flex items-center gap-2">
              ...
              <Button
                ...
                size="icon"
                className="lg:hidden"
                aria-label={mobileOpen ? "Navigation ausblenden" : "Navigation einblenden"}
              ...
              {/* Desktop toggle – stays visible in both states */}
              <Button
                variant="ghost"
                size="icon"
                className="hidden lg:inline-flex"
                aria-label={expanded ? "Navigation ausblenden" : "Navigation einblenden"}
                aria-expanded={expanded}
                aria-controls={DESKTOP_NAV_ID}
                onClick={toggle}
              >
                {expanded ? (
                  <PanelLeftClose className="icon-brand size-5" />
                ) : (
                  <PanelLeftOpen className="icon-brand size-5" />
                )}
              </Button>
